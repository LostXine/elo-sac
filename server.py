...
# Score from DrQ paper
baseline = {
    "cartpole swingup":759,
    "cheetah run": 344,
    "ball_in_cup catch": 913,
    "walker walk": 612,
    "reacher easy": 601,
    "finger spin": 901,
}
...
class ParaManager:
    def __init__(self, args):
        ...
        # type, min, max
        self.paras = [
            (int, 84, 116, 'Tgt random shift'), # aug_size
            (int, 84, 116, 'Random shift'), # rl_aug_size
            (float, -4, 4, 'CURL'), # weights
            (float, -4, 4, 'DINO'),
            (float, -4, 4, 'AutoEncoder'),
            (float, -4, 4, 'Predict FR Balanced'),
            (float, -4, 4, 'Extract AR Balanced'),
            (float, -4, 4, 'Rotation CLS'),
        ]
        ...
        # initialization
        n_weights = len(self.paras) - 2
        for i, j in enumerate([1/16, 1/8, 1/4, 1/2, 1]): # aug 86, 88, 92, 100, 116
            self.p_pos[n_weights * i:n_weights * (i + 1), 2:] = np.eye(n_weights, n_weights) * 0.5 # 0.5 will be mapped to 0, which means weight=1
            self.p_pos[n_weights * i:n_weights * (i + 1), :2] = j
    ...
    def post_para(self, data_str):
        try:
            ...
            vals = []
            local_score = {}
            for env in p["results"]:
                results_dist = np.array(p["results"][env])
                # score = scipy.stats.trim_mean(results_dist, proportiontocut=0.25, axis=None)
                score = np.mean(results_dist)
                norm_score = score / baseline[env]
                vals.append(norm_score)
                local_score[env] = score
                local_score['norm_' + env] = norm_score
            ...
            if p_val > self.p_best_val[p_idx]:
                ...
                logging.info("Local best score: " + ','.join([f'{i}: {local_score[i]:2f}'for i in local_score]))

            if p_val > self.g_best_val:
                self.g_best_val = p_val
                self.g_best_dict = local_score
                ...
                logging.info("Global best score: " + ','.join([f'{i}: {local_score[i]:2f}'for i in local_score]))
...
def run():
    ...
    parser.add_argument('--repeat', type=int, help='test one set of parameters multiple time. (default: 3)', default=3)
    parser.add_argument('--timeout', type=int, help='particle busy state reset timeout (unit: hour, default: 12)', default=36)
    parser.add_argument('--path', type=str, help='where to save search result file (default: evolve.npy)', default='./server/evolve.npy')
    parser.add_argument('--log', type=str, help='where to save log file (default: search-log.log)', default='./server/search-log.log')
